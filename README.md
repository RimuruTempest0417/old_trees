...
25. [分頁稽核：每一顆按鈕都真的點一次](#二十五分頁稽核每一顆按鈕都真的點一次)
...
| **實地考察** | 現場記錄表單（古樹編號、觀察日期、記錄者、天氣、健康狀況、**樹皮狀況勾選**〈剝落／黴斑／白色鹽類結晶／無明顯異常〉、**周邊環境勾選**〈鄰近馬路／建築物／排水口／水泥覆蓋／裸露土壤／其他〉、**樹穴水泥覆蓋範圍五級**、樹高／胸徑／冠幅、立地環境、病蟲害與損傷、**現場照片（手機拍照 → 前端壓縮 → 上傳 Storage）**、**GPS 誤差半徑比對**〈一鍵比對官方逐株座標，顯示距離與定位精度〉）、紀錄清單（含照片縮圖與「位置比對」欄）與 CSV 匯出（含結構化欄位與 GPS 兩欄）、現場檢查清單與安全提醒；頁面下方只列「**還在規劃中**」（多人協作審核；2026-09-26 起不再顯示已上線清單，做完的事也不會留在規劃中）。紀錄存於 `field_records` 表、照片存於 Storage（`field-photos` bucket），官方名錄不會被覆寫 |
...
| **列印** | 把資料變成可帶去現場的紙本：**單株 A4 樹木檔案卡**（官方照片、13 項基本資料、形態描述、現場查核清單、二維碼）、**實地考察紀錄單**（可帶入某株基本資料或空白表，含量測表格與勾選清單）、**路綫資料冊**（封面含站點示意圖與比例尺，之後每站一頁）、**政策方案摘要**（9 頁：總覽、三個方向各 2 頁、來源索引、已知缺口）；按「列印／存成 PDF」即可在列印對話框選擇「另存為 PDF」，A4 一頁一株 |
...
| **GPS 誤差半徑比對**（v0.15.0） | 實地考察表單按一下就用目前位置比對**官方逐株座標**：顯示距離、手機定位精度與判定（位置相符 ≤30 公尺／可能不是這一株／精度不足僅供參考），距離與精度一併存進紀錄並可匯出 CSV；官方沒有座標的樹會明說「無法比對」，不會假裝成功 |
| **政策方案** | **要求 4 的網站化呈現**：城市綠化／環保節能／文旅文創三個方向，每個方向寫出**政策依據**（法令資料庫、市政署、環保局、文化局、旅遊局、教青局、立法會與兩篇學術文獻，共 23 個來源，可點開原文）、**具體行動**（做什麼／誰負責／合作單位／在哪裡／成功指標／期程／成本概念，共 10 項）、**以本站 658 株官方資料排出的優先順序**，以及**「本頁沒有的東西」**（官方沒有城市樹木降溫與逐株碳匯數字等 7 項缺口）；支援複製摘要與匯出行動清單 CSV |
...
| **要求 4 政策型設計方案**（城市綠化／環保節能／文旅文創） | **「政策方案」分頁**：每個方向的政策依據（附官方出處）、具體行動（負責單位、地點、期程、成本概念、成功指標）與優先順序理由；另有可列印的 A4 方案摘要（列印分頁）與行動清單 CSV |
...
自動化稽核 `scripts/ui-audit.sh`（需另備注入式量測頁）以無頭 Chrome 在 360／390／414／834／1440 px 與深淺色共 120 種組合下量測頁面捲動寬度、元素溢出、觸控目標高度與輸入框字級，全部 `pageOverflow = 0`。
...
│   ├── routes/             18 個端點的 handler（health／trees／tree／stats／priority／monitoring／env-chem…）
...
| `field_records` | **實地考察紀錄**：古樹編號、觀察日期、記錄者、天氣、健康狀況、**樹皮狀況（陣列）**、**周邊環境（陣列）**、**水泥覆蓋範圍**、樹高／胸徑／冠幅、立地環境、病蟲害與損傷、外部照片網址、**Storage 照片路徑（陣列）**、座標、**GPS 定位精度與與官方座標距離**、建立時間 |
...
| `GET /api/policy` | **政策方案（要求 4）**；`direction=green｜energy｜tourism`（單一方向，未知回 404）、`all=1`（完整內容＋來源索引）、`gaps=1`（只取「本頁沒有的東西」）。回傳 `directions`／`gaps`／`summary`（統計、方法、**缺漏來源自我檢查**）與 `hash`。每一條政策與行動都內嵌 `source`（完整出處物件）與 `basis_sources`（政策依據）；**只接受 GET／HEAD，其他方法回 405** |
...
| `tests/field.test.js` | 實地考察：API 清單與新增、輸入驗證（必填、健康值、數值範圍、長度截斷）、結構化欄位（合法值保留、非法值回 400 並列允許值）、`schema.sql`／`init.sql` 含 `field_records`、前端分頁與地圖入口串接；**已上線清單整段不得再出現、規劃中不得殘留做完的事（GPS 比對已實作）、刪說明段落時功能欄位不得跟著被刪**；**三份選項清單（前端／後端／資料庫 CHECK）一致性**；**A4 考察單的版面約束**（新勾選格必須在右欄並橫向展開，否則單頁會被撐成三頁）；`/api/photo` 只接受 POST、示範模式不假裝上傳成功 | 16 |
...
**要求 4** 要求挑選「城市綠化／環保節能／文旅文創」任一方向（或一個以上），配合澳門特區政府政策，
提出可切實執行的設計方案。這一節說明它怎麼被做成網站的第十二個分頁（`#/policy`）。

### 19.1 三個方向與內容結構

| 方向 | 目標（摘要） | 政策依據 | 具體行動 |
| --- | --- | --- | --- |
| **城市綠化** | 以法定名錄為清單、以綠地增量為載體，把 658 株古樹變成城市綠地網絡的節點 | 6 條 | 3 項（G1 綠地增量選址對接古樹棲地／G2 一樹一檔案的公開版／G3 樹穴透氣與鋪面改善清單） |
| **環保節能** | 把古樹當成城市的被動降溫基礎設施，用樹蔭減少空調用電 | 7 條 | 3 項（N1 熱點遮蔭改善／N2 建築節能×綠化組合試點／N3 碳匯誠實科普） |
| **文旅文創** | 把古樹變成可導覽、可體驗、可帶走的內容 | 8 條 | 4 項（T1 世遺古樹導覽支線／T2 古樹集章路線／T3 90 分鐘研學模組／T4 文創產品與收益回饋） |

每一項行動都寫明七件事：**做什麼、誰負責、一起做、在哪裡、怎麼算成功、期程、成本概念**，
並附上政策依據（可點開法令資料庫或官方文件的原文）。

### 19.2 為什麼先做這幾項（優先順序來自本站資料）

政策的優先順序不是憑感覺排的，而是用本站 658 株的官方資料算出來的。例如：

- 官方綠地有 **55.6% 集中在路環**，而古樹密度最高的**風順堂區、望德堂區都是 123.3 株／平方公里**——最需要綠地舒緩的地方，正是綠地最少的地方（G1）。
- 2025 年澳門**酷熱天氣日數 37 日**（較氣候平均值多 5.7 日），而本地發電佔溫室氣體排放 26.6% 又是降溫用電的去向（N1、N2）。
- 第 4/2024 號行政法規已要求維持**白鴿巢前地、亞婆井前地、媽閣廟前地、民國大馬路**的樹種與樹木佈局——導覽支線的解說點位就在這幾處（T1）。

### 19.3 誠實原則（與化學視角同一套規矩）

1. **每一條政策、每一個數字都附官方出處**：23 個來源全部是官方網域（`bo.dsaj.gov.mo` 法令資料庫、市政署、環保局、文化局、旅遊局、教青局、立法會、政府入口網站）或經 Crossref 核對書目資料的學術文獻。
2. **官方查不到的一律寫在「本頁沒有的東西」**（7 項），例如：澳門**沒有官方公開的城市樹木降溫度數**、**沒有逐株碳匯公噸數**；也不拿外國數字當澳門數字。
3. **國際研究只作定性引用**：Bowler et al. (2010) 與 Yu et al. (2022) 的書目資料已用 Crossref 核對，但出版方網站封鎖自動抓取、本站未能取得全文核對其中的具體度數，因此**不引用其數字**並在缺口清單說明。
4. **私有地古樹的資訊有限**也照實寫出來（法律課予權利人維護義務，但沒有公開清冊）。

### 19.4 資料流與檔案

```
data/policy.json            政策方案的資料層（人工整理，每一項附 source 指向 sources[].id）
  ↓ node scripts/gen-policy.mjs（npm run build:policy；npm run build:data 已納入）
data/policy-data.js         產生檔（附內容雜湊；改了 JSON 忘了重跑，npm test 會紅燈）
  ↓
lib/policy.js               純函式：summary／directions／directionById／gaps／sourcesList／sourcesIndex
  ↓
lib/routes/policy.js        GET /api/policy（?direction=｜?all=1｜?gaps=1；只接受 GET／HEAD）
  ↓
public/js/policy.js         第十二個分頁 #/policy（政策表、行動卡、缺口、來源索引、CSV）
public/js/card.js           policySheetHtml() → #/card?mode=policy 的 A4 方案摘要（9 頁）
```

### 19.5 驗證方式

```bash
node --test tests/policy.test.js                     # 18 項
node --test tests/*.test.js                          # 21 組，共 308 項
node scripts/gen-policy.mjs --check                  # 產生檔是否同步
bash scripts/ui-audit.sh <port>                      # 120/120 組（含 #/policy）
python3 scripts/card-pdf.py <port>                   # 含 policy：9 頁、A4、文字可抽取

# 政策方案頁（實跑 JavaScript 後取回 DOM 狀態）
node scripts/cdp-check.mjs "http://127.0.0.1:3473/#/policy" --wait "城市綠化" --wait "文旅文創" \
  --eval "document.querySelectorAll('#view-policy .policy-action').length"      # → 10

# A4 摘要的頁面高度（快很多的前置檢查：每張 .card-page 都不得超過 292mm）
node scripts/cdp-check.mjs "http://127.0.0.1:3473/#/card?mode=policy" --wait "共 9 頁" \
  --eval "[...document.querySelectorAll('.card-page')].map(el => Math.round(el.getBoundingClientRect().height * 25.4 / 96 * 2))"
```

> **A4 版面注意**：政策方案的內文很長，一個方向塞進一頁會溢出（實測 13 頁）。
> 現在每個方向拆成「政策依據」與「具體行動」兩頁，來源索引與缺口各自一頁，共 9 頁；
> 列印時長句會截短並加刪節號（完整內容留在網頁版），這是刻意的取捨。

---

## 二十、GPS 誤差半徑比對

實地考察最容易出錯的不是量測，而是**站錯樹**。這一版把「有沒有走到正確的那一株」變成可計算、可記錄、
可事後核對的事：用官方逐株座標當基準，與手機定位比對。

### 20.1 為什麼不能只看距離

手機定位本身帶著誤差半徑。在空曠處可能只有 ±5 公尺，但站在樹蔭下、建築物旁或室內可能是 ±80 公尺——
只看「距離 8 公尺」就宣布「位置相符」，等於把定位誤差當成自己的判斷力。因此判定同時看兩件事：

| 情況 | 判定 | 畫面 |
| --- | --- | --- |
| 距離 ≤ **30 公尺**且定位精度 ≤ **±50 公尺** | 位置相符 | 綠色 |
| 距離 > 30 公尺 | 可能不是這一株（請核對樹號與現場立牌） | 紅色 |
| 定位精度差於 ±50 公尺 | 距離僅供參考（請走到空曠處再測） | 橘色 |
| 官方沒有這一株的座標 | 無法比對 | 灰色 |

30 與 50 這兩個門檻寫在 `lib/geo.js`（`GPS_MATCH_RADIUS_M`／`GPS_ACCURACY_LIMIT_M`）與
`public/js/field.js` 兩處；前端不能 import 後端的 `lib/`，所以這是刻意的複製，
`tests/gps.test.js` 會逐項比對兩邊的數值與球面距離公式（地球半徑、`asin` 形式），改一邊沒改另一邊會紅燈。
...
1. 現場座標寫進 `lat`／`lon`；
2. 手機回報的精度寫進 `gps_accuracy_m`；
3. 算出的距離寫進 `gps_distance_m`（沒有官方座標時**留空**，不補 0）；
4. 紀錄清單多一欄「位置比對」（`距 5 m／±8 m` 這樣顯示，距離在 30 公尺內為綠字）。

三處的值域必須一致：前端欄位名、`lib/repo.js` 的 `GPS_LIMITS`（0–1000／0–100000 公尺）、
`supabase/schema.sql` 的 `field_records_gps_check`；`tests/gps.test.js` 逐項比對。

也可以手動輸入緯度／經度（現場沒開定位權限時），只要官方座標在，比對照樣算得出來。
紙本考察單也留了「現場 GPS 座標」「定位精度」「與官方座標距離」「位置比對（□ 相符 □ 可能不是這一株）」欄位，
現場沒網路時先在紙上寫，回教室再輸入。

### 20.3 順手修掉的一個小問題

表單的空欄位送出去是空字串 `''`，而紀錄清單原本用 `!= null` 判斷有沒有值——
空字串不等於 `null`，於是「沒有量胸徑」會顯示成「胸徑 0.0 cm」。
**沒量到卻看起來像量到 0**，報告引用時會被誤讀，所以這一版加了 `cleanNums()`／`hasNum()`：
空字串一律視為「沒有量」，存成 `null`、畫面顯示「—」。

### 20.4 驗證方式

```bash
node --test tests/gps.test.js        # 11 項（判定規則、邊界、前後端一致、三處一致、紙本欄位）
node --test tests/*.test.js          # 24 組，共 359 項

# 用 CDP 模擬手機定位，真的按下按鈕看判定（--geo 緯度,經度,精度）
node scripts/cdp-check.mjs "http://127.0.0.1:3475/#/field?tree=66" --wait "比對：" \
  --geo 22.159097,113.545271,8 --eval "document.querySelector('#field-gps').textContent"
#   → 與官方座標相距 5 公尺，在 30 公尺比對半徑內——位置相符（定位精度約 ±8 公尺）。
node scripts/cdp-check.mjs "http://127.0.0.1:3475/#/field?tree=66" --wait "比對：" \
  --geo 22.161747,113.545271,10 --eval "document.querySelector('#field-gps').textContent"
#   → 與官方座標相距 300 公尺，超出 30 公尺比對半徑——可能不是這一株，請核對樹號與現場立牌。
node scripts/cdp-check.mjs "http://127.0.0.1:3475/#/field?tree=66" --wait "比對：" \
  --geo 22.159097,113.545271,120 --eval "document.querySelector('#field-gps').textContent"
#   → 定位精度約 ±120 公尺，比 ±50 公尺差，與官方座標相距 5 公尺僅供參考——請走到空曠處再測一次。
```

`--geo` 是 v0.15.0 加進 `scripts/cdp-check.mjs` 的選項：它會同時對瀏覽器授權定位並覆寫座標
（少了授權，無頭 Chrome 的 `getCurrentPosition` 會直接進 error callback，永遠測不到判定）。

> **部署提醒**：`gps_accuracy_m`／`gps_distance_m` 是新的資料庫欄位，
> 要在 Supabase SQL Editor **整份貼上 `supabase/init.sql` 並 Cmd/Ctrl+A 全選後 Run**，
> 有紀錄的裝置才存得進去（沒重跑的話，寫入時 `/api/health` 會提示缺少欄位）。

---

## 二十一、優先保育行動建議（把名單變成工作清單）

優先保育名單回答「哪一株最急」，但拿到名單的人接著要問「那我明天先做什麼」。這一節把名單接上
**可由官方資料推導的建議行動**——十一條規則寫在 `lib/actions.js`，同一個條件永遠得到同一個建議：

| 建議時程 | 行動 | 目前株數 | 條件 |
| --- | --- | --- | --- |
| 立即處理 | 排入專家複查與搶救復壯評估 | 21 | 官方健康狀況＝瀕危 |
| 立即處理 | 加做落枝風險評估、設現場告示並加密巡查 | 1 | 區位風險高且優先分數 ≥ 75 |
| 今年內 | 現場量測冠幅並補登官方缺漏欄位 | 591 | 官方冠幅值缺漏 |
| 今年內 | 檢視法定保護牌、圍欄與解說牌 | 7 | 官方分級一級／二級 |
| 今年內 | 樹體支撐、樹穴與立地環境改善評估 | 7 | 樹齡 ≥ 300 年 |
| 今年內 | 核對《古樹名錄》與官方現行值的差異並註記 | 6 | 官方現行值與名錄不同 |
| 今年內 | 補拍全株照與樹幹特寫 | 1 | 官方照片缺漏（該株原始影像已下架） |
| 持續追蹤 | 列入稀有母樹保護與種子採集評估 | 67 | 全澳同種 ≤ 3 株 |
| 持續追蹤 | 納入文化導覽路線與解說點 | 7 | 樹齡 ≥ 300 年且列入名錄 |
| 持續追蹤 | 維持每年至少一次樹體檢查 | 4 | 官方健康「一般」且分數 ≥ 60 |

（株數由官方資料即時計算，資料更新時會變動；`gps-fix`「以手機 GPS 校正座標」目前 0 株——
官方 658 株座標皆為逐株實測，沒有需要校正的對象，這一條留著是為了資料欄位改變時自動接手。）

- 每一條都附**依據**（法規名稱、指引或可回溯的本站統計）與**為什麼是這一株**（例如「官方健康狀況為『瀕危』」）。
- 缺值不補：官方沒有的欄位會變成「去現場補量／補拍」的建議，而不是推估值。
- 紙本列印 `#/card?mode=actions`：第 1 頁行動總表，之後每頁 40 列逐株清單
  （「立即處理」全部列出，其餘行動只列代表株，完整名單走 CSV，避免紙本變成幾十頁）。
- 明確聲明：這是**巡查與資料整理的優先順序**，不是樹木醫學診斷或施工方案；
  真正處置由具職權的公共部門與專業人員評估後決定。

API：`GET /api/priority`（附 `actions` 與每株 `advice`）、`GET /api/priority?all=actions`（含完整成員清單，供 CSV 與列印）。

---

## 二十二、科普數據導讀（用同一份官方資料讀文章）

科普文章原本只有文字與來源；這一版在每篇文章開頭加上「**用本站數據讀懂這篇**」——
把文章的論點接上實際數字，而且數字由後端以同一份官方資料**即時計算**（`lib/knowledge-guides.js`），
官方資料更新時文章不會講出過期的數字。

- 12 篇文章各有 3–4 條導讀，共 **38 條**：每條都由指標模板產生（`{v}` 由後端填值），
  標示是「**官方資料**」（名錄、自然網、環境年報）或「**本站統計**」（照片可用數、名錄與官方值差異株數）。
- 每條都附站內深連結（例如「657 株有可用的官方照片」→ `#/card`、「6 株名錄與官方值不同」→ `#/monitoring`）。
- 缺值不顯示假句子：算不出值的條目直接略過，不會出現「— 株」或 `NaN`。
...
## 二十五、分頁稽核：每一顆按鈕都真的點一次

單元測試驗的是資料與演算法，`cdp-check.mjs` 驗的是「畫面上有沒有出現某段文字」——
兩者都不會告訴你「**按下去有沒有反應**」。數據分析頁的擬合模型切換鈕曾經按了完全沒變化，
而當時測試全綠、頁面文字也完整，這類問題只有真的去點才會出現。

### 25.1 怎麼跑

```bash
npm run dev                                   # 另開一個終端
node scripts/page-audit.mjs http://127.0.0.1:3000 --mode all --out /tmp/page-audit.md
node scripts/page-audit.mjs http://127.0.0.1:3000 --mode clicks --views analytics   # 只跑一頁
node scripts/page-audit.mjs http://127.0.0.1:3000 --mode overflow --width 1440      # 只掃版面
```

- **互動稽核**：16 個網址（12 個分頁，加上 `#/map?tree=`、`#/monitoring?tree=`、`#/field?tree=`、
  `#/card?mode=form` 等深連結）逐一把 `button`、`[role=button]`、`summary`、`a.btn`、
  `.tab`、`select`、checkbox／radio 都點一次。每個元素都在**重新載入的乾淨頁面**上點，
  所以彼此不汙染，錯誤也能明確歸屬到某一個元素。
- **判定**：`✅ 有反應`（hash 改變／面板開關／toast／列印／下載／剪貼簿／內容更新 任一發生）、
  `✅ 預期無變化`（重按目前已選取的分頁或項目、必填欄位未填被瀏覽器原生驗證擋下、
  「產生」類按鈕重新產生相同內容等）、`⚠️ 沒反應`、`❌ 報錯`、`— 外部連結只驗網址`。
  沒反應的會**自動重載再點一次**確認，兩次都沒有變化才記下來。
- **狀態指紋**：`__domHash` 除了文字長度與元素數，還包含各圖表的資料集、表單值與勾選狀態、
  `<details>` 開合數量、`.active` 選取數量 —— 少了這些，「勾了勾選框」「展開說明」
  「換了路綫卡片」都會被誤判成沒反應。
- **時間上限**：`--budget <秒>`（預設 2400），逾時會停下來並在報告開頭註明哪一段沒跑完。
- **圖表切換要能被看出來**：canvas 的內容不會反映在 DOM 上，所以狀態指紋
  （`__domHash`）會把每張圖表的資料集數量、點數與最後一條曲線的第一點一起算進去，
  否則「換了模型、曲線真的變了」會被誤判成沒反應。
- **版面稽核**：12 個分頁 × 5 種寬度（1440／1024／768／430／390），找出「文字溢出容器」
  與「整頁橫向捲動」，並跳過刻意可捲動的容器（`.table-wrap`、`.chart-box`、`.card-scroll`）。
- **不會把檔案下載到你的電腦**：CDP 把下載行為設為 `deny`；`window.print`、`window.open`、
  `navigator.clipboard.writeText`、`URL.createObjectURL` 都在載入前被替換成計數器，
  只記錄「有沒有被呼叫」，不會真的列印、開視窗或存檔。

### 25.2 v1.0.0 修的實測問題（都是這支稽核抓到的）

1. **切換擬合模型／線條沒有反應**（`public/js/charts.js`）
   Chart.js 不允許同一塊 canvas 同時掛兩張圖；原本每個建構函式都直接 `new Chart(canvas)`，
   第二次以後會丟出「Canvas is already in use」並保留舊圖，畫面看起來就是「點了沒反應」。
   修法：新增 `mount()`，建立前先 `destroy()` 同一塊 canvas 上的舊圖；所有圖表都走這條路。
2. **樹木資訊面板在換頁後不會自動關閉**（`public/js/app.js`）
   面板掛在共用的 `#modal` 上，切換分頁時沒有人去關它，於是「優先保育」的樹木卡片會留在
   「列印」頁上。修法：`showView()` 進場先 `closeModal()`。
3. **地圖：從 `#/map?tree=` 進來後再按「套用篩選」會丟 TypeError**（`public/js/map.js`）
   `TypeError: Cannot use 'in' operator to search for '_leaflet_id' in undefined`。
   `markercluster` 的 `zoomToShowLayer()` 會在分群圖層註冊 `moveend`／`animationend` 回呼，
   之後 `clearLayers()` 重新加入標記時，那些回呼拿到的是已被移除的標記。
   修法：改用自製的 `openMarker()`（`setView` 到 17 級後開泡泡），並關閉分群動畫。

### 25.3 文字換行與版面（版面稽核量到的）

- **篩選表單的兩欄格線溢出 30px**：`.grid-2` 原本是 `minmax(320px, 1fr)`，容器只要比
  「320px×2＋間距」窄一點就會整排溢出 → 改 `minmax(min(320px, 100%), 1fr)`。
- **檔案卡的照片來源圖說溢出 97px**：一行長網址不會斷行 → 對 `figcaption`、`.src-link`、
  `.muted`、`.tiny` 等加 `overflow-wrap: anywhere`。
- **優先保育名單的表格**：`table.table` 過去只有瀏覽器預設樣式（沒有任何 CSS），
  11 欄的內容擠在一起。現在補上列線、欄線、斑馬紋、表頭底色，並把「主要理由」改成清單、
  「建議行動」每一條之間加分隔線與間距。
- **手機版名單改為卡片**：700px 以下 `table.table` 改成一株一張卡（用 `data-label` 當欄位名），
  不必左右滑動就看得到理由與建議。桌面版不受影響。
- **`cdp-check.mjs` 新增 `--waitjs`**：等自訂 JS 條件成立才截圖。
  之前用 iframe 量手機寬度時，截圖常常拍在「載入中」那一刻——判定與截圖必須分開、截圖放後面。

### 25.4 v1.0.0 實測結果

| 項目 | 指令 | 結果 |
|---|---|---|
| 互動稽核 | `page-audit.mjs --mode all` | 16 個網址、**501 個可點元素**：458 有反應、41 預期無變化、2 只驗外部連結、**0 沒反應、0 報錯** |
| 版面稽核 | 同上（12 分頁 × 5 寬度） | 整頁橫向捲動 **0px**、無文字溢出、無內容被容器裁切 |
| 語法檢查 | `npm run check` | 92 個檔案通過 |
| 單元測試 | `npm test` | 25 組、369 項全過 |
| 介面稽核 | `ui-audit.sh <port>` | 12 分頁 × 5 寬度 × 深淺色 ＝ 120/120 |
| Service Worker | `build-sw.mjs` | 預載 60 項、版本 `v1.0.0` |
