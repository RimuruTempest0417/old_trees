...
export default handler(async (p) => {
  ...
  if (p.tree !== undefined && p.tree !== '') {
    ...
    const s = buildSeries(t, {
      listing: { grade: t.listing_grade, health: t.listing_health },
      snapshots: SNAPSHOTS,
  ...
  let rows = series.map((s) => ({
    ...
    changes: s.steps
      .filter((x) => x.health_change || x.grade_change || x.diameter_delta || x.height_delta)
      .map((x) => ({
        date: x.to, from: x.from_label, to: x.to_label,
        health: x.health_change, grade: x.grade_change,
