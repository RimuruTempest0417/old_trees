...
function normalizeTreeRow(r) {
  return {
    ...r,
    ...officialFirst(r),          // 顯示用的分級／健康狀況一律官方現行值；名錄值放 listing_*
...
export async function allTrees() {
  if (DATA_SOURCE === 'supabase') {
    ...
    const rows = await must(await sb().from('v_trees')
      .select('tree_no,age_years,height_m,species,parish,site,health,grade,official_health,official_grade,'
        + 'lat,lon,diameter_cm,official_loc')
      .limit(2000));
    return rows.map((r) => ({
      ...r,
      ...officialFirst(r),          // 分級／健康狀況一律用官方現行值（見 officialFirst 說明）
      age_years: num(r.age_years), height_m: num(r.height_m),
...
export const API_VERSION = 'v0.11.1';
